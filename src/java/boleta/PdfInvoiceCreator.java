...
public class PdfInvoiceCreator {
    ...
    public void createTnc(List<String> TncList, Boolean lastPage, String imagePath) {
        if (lastPage) {
            ...
            tb.addCell(new Cell().add("TERMINOS Y CONDICIONES\n").setBold().setBorder(Border.NO_BORDER));
    ...
    public void createProduct(List<Product> productList) {
        ...
        float subTotal = redondearADosDecimales(getTotalSum(productList));
        float igv = redondearADosDecimales(subTotal * 0.18f);
        float totalNeto = redondearADosDecimales(subTotal + igv);
        ...
        threeColTable3.addCell(new Cell().add("SubTotal").setTextAlignment(TextAlignment.CENTER).setBorder(Border.NO_BORDER));
        threeColTable3.addCell(new Cell().add(String.valueOf(subTotal)).setTextAlignment(TextAlignment.RIGHT).setBorder(Border.NO_BORDER)).setMarginRight(15f);
        //IGV
        threeColTable3.addCell(new Cell().add("").setBorder(Border.NO_BORDER)).setMarginLeft(10f);
        threeColTable3.addCell(new Cell().add("IGV").setTextAlignment(TextAlignment.CENTER).setBorder(Border.NO_BORDER));
        threeColTable3.addCell(new Cell().add(String.valueOf(igv)).setTextAlignment(TextAlignment.RIGHT).setBorder(Border.NO_BORDER)).setMarginRight(15f);
        //TOTAL (NETO)
        threeColTable3.addCell(new Cell().add("").setBorder(Border.NO_BORDER)).setMarginLeft(10f);
        threeColTable3.addCell(new Cell().add("Total").setTextAlignment(TextAlignment.CENTER).setBorder(Border.NO_BORDER));
        threeColTable3.addCell(new Cell().add(String.valueOf(totalNeto)).setTextAlignment(TextAlignment.RIGHT).setBorder(Border.NO_BORDER)).setMarginRight(15f);
    ...
    public List<Product> getDummyProductList() {
        ...
        productList.add(new Product("apple", 5, 200));
        productList.add(new Product("kiwi", 2, 90));
        return productList;
    }

    public static float redondearADosDecimales(float valor) {
        // Multiplicar por 100 para conservar dos decimales
        float valorMultiplicado = valor * 100;
        // Redondear al entero más cercano
        float valorRedondeado = Math.round(valorMultiplicado);
        // Dividir por 100 para obtener el valor redondeado a dos decimales
        return valorRedondeado / 100;
    }
    ...
    public void createTableHeader(ProductTableHeader productTableHeader) {
        ...
        threeColTable1.addCell(new Cell().add("Descripcion").setBold().setFontColor(Color.WHITE).setBorder(Border.NO_BORDER));
    ...
    public File generatePdfFile(List<Product> productList, List<String> TncList, String imagePath, HeaderDetails header, AddressDetails addressDetails, ProductTableHeader productTableHeader, Boolean lastPage) throws IOException {
        // Crear el archivo en la ubicación seleccionada por el usuario
        ...
        createTnc(TncList, lastPage, imagePath);
        // Cerrar el documento
        document.close();
        // Retornar el archivo
        return pdfFile;
